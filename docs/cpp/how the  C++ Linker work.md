...
so the first stage(阶段) is actually compiling our source files and I actually made an entire video on that .
so go check that out link in the description below once we've compiled our files we need to go through a process called  Linker .
now the primary focus of linking is to find
